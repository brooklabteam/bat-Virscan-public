...
figS2 <- ggplot(es.merge) + theme_bw() + theme(panel.grid  = element_blank(),
  ...
  #facet_grid(virus_family~., scales="free_y") +
  facet_wrap(virus_family~., scales="free_y") + 
  facet_nested(virus_subfamily + virus_genus~., scales = "free_y", switch = "y") + coord_cartesian(expand=F)
  #gnewscale::new_scale_fill() + coord_cartesian(xlim=c(.75, (max(es.merge$xlab)+.5)), expand = F)+
...
#could try with picorna and rhabdo which also look good and comment on them in the text
...
pheat <- ggplot(pa.para) + theme_bw() + theme(panel.grid = element_blank(),
  ...
  facet_nested(virus_subfamily + virus_genus~., scales = "free_y", switch = "y") +
  scale_fill_viridis_c() + scale_y_discrete(position = "right") + coord_cartesian(expand = F)
